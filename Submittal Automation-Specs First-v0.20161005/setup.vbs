...
'Excel Documents
	...
	'Table of Contents
		'Setup Excel for ToC
		Set tocWorkbook = allExcel.Workbooks.Open(miscPath + "\Table of Contents.xlsx")
		...
		'Fill in Product Info  ~ In order to change order from original, Changed the splitPath call up one and 4 to 0.  ex. splitPath(2) changed to splitPath(3).
		...
		For Each targetfile In files
			If UCase(ccFSO.GetExtensionName(targetfile.name)) = "PDF" Then
				...
				If ubound(splitPath) = 4 Then
					tocWorksheet.Cells(rowNumber,4) = splitPath(0)
				End If
				'Description
				tocWorksheet.Cells(rowNumber,6) = splitPath(3)
				If debug Then
				...
				End If
				'Model/type/color
				tocWorksheet.Cells(rowNumber,8) = splitPath(4)
				If debug Then
				...
				End If
				'Part Number
				tocWorksheet.Cells(rowNumber,10) = splitPath(2)
				'Manufacturer
				tocWorksheet.Cells(rowNumber,12) = splitPath(1)
